...
pub struct LchGradientDescriptor {
    colors: vec::Vec<Lch<white_point::D65, f64>>,
    positions: vec::Vec<f64>,
    wrap: bool,
}

impl LchGradientDescriptor {
    pub fn new(colors: vec::Vec<Lch<white_point::D65, f64>>, wrap: bool) -> LchGradientDescriptor {
        let rng = if wrap && colors.len() > 1 {
            0..colors.len() + 1
        } else {
            0..colors.len()
...
mod tests {
    ...
    #[test]
    fn test_generate_lch() {
        let grad_desc = color::LchGradientDescriptor::new(
            vec![
